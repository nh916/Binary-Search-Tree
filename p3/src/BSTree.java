...
//            halfOfBST.add(key.get(i));
            ...
            insert(root, new BSTNode<E>((E) key.get(i)));
        }
    }
...
//            halfOfBST.add(key.get(i));
            deleteKey(new BSTNode(key.get(i)));
...
//    todo this is currently useless because I cannot be sure that the tree is verified from my driver class.
    ...
    private boolean verify(BSTNode<E> root, Integer minValue, Integer maxValue) {
        ...
        assert((Integer)root.getData() >= minValue && (Integer)root.getData() < maxValue && verify(root.getLeft(), minValue, (Integer) root.getData()) && verify(root.getRight(), (Integer) root.getData(), maxValue));
        return true;
    }
    ...
    public void deleteKey(BSTElement key) {
        root = deleteRec(root, key);
    }


    //     A recursive function to insert a new key in BST
    private BSTNode<E> deleteRec(BSTNode<E> root, BSTElement key){
...
//         Otherwise, recur down the tree
        ...
        // to be deleted
            ...
            // in the right subtree)
                ...
                root.set((Integer) minValue(root.getRight()));

                // Delete the inorder successor
                root.setRight(deleteRec(root.getRight(), (BSTElement) root.getData()));
            }
        }
    ...
    //    find the successor
    private E minValue(BSTNode<E> root){
