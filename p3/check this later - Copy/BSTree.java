...
//    second verify that uses inOrder traversal that checks to see if the tree is verified
    ...
    public boolean verifyInorder(BSTNode root){
        if (root == null){
            return true;
        }
        else {
    ...
    public boolean compare(BSTNode parent, BSTNode child, boolean isLeft){
        if (child == null){
            return true;
        }
        if (isLeft){
...
//        System.out.println("verification: " + test.verify(test.root));
